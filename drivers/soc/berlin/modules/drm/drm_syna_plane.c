...
static int syna_plane_set_surface_check(struct drm_crtc *crtc, struct drm_plane *plane,
				struct drm_framebuffer *fb,
				const uint32_t src_x, const uint32_t src_y)
{
	struct syna_plane *syna_plane = to_syna_plane(plane);
	...
	return syna_vpp_set_surface_check(plane->dev,
			syna_crtc->syna_reg,
			syna_plane->plane_id,
			fb,
			src_x, src_y);
					  ...
					  syna_drm_plane_state *state)
{
	...
	ret = syna_plane_set_surface_check(plane_state->crtc, plane, fb,
