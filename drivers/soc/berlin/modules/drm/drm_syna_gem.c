...
static void syna_gem_free_buf(struct drm_gem_object *obj, struct syna_gem_object *syna_obj)
{
	...
	if (syna_obj->shm_handle.handle) {
		...
		VPP_MEM_FreeMemory(dev_priv->mem_list, syna_obj->mem_type, &syna_obj->shm_handle);
...
{
	struct syna_gem_object *syna_obj = to_syna_obj(obj);
		struct sg_table *sgt, *sgt_dst;
	struct scatterlist *sg, *sg_dst;
	int ret, i;
	...
	if (syna_obj->sgt) {
	...
	} else if (syna_obj->shm_handle.table) {
		sgt_dst = syna_obj->shm_handle.table;
		DRM_DEBUG_PRIME("exporting sgt table for dmabuf %p, sgt entries:%d\n", syna_obj->shm_handle.handle, sgt_dst->nents);
		sgt = kzalloc(sizeof(*sgt), GFP_KERNEL);
		if (!sgt) {
			DRM_ERROR("failed to allocate sgt\n");
			return ERR_PTR(-ENOMEM);
		}
		ret = sg_alloc_table(sgt, sgt_dst->nents, GFP_KERNEL);
		if (ret) {
		...
		sg_dst = sgt->sgl;
		for_each_sg(sgt_dst->sgl, sg, sgt_dst->orig_nents, i) {
			sg_set_page(sg_dst, sg_page(sg), sg->length, sg->offset);
						      ...
						      struct sg_table *sgt)
{
	...
	for_each_sg(sgt->sgl, sg, sgt->nents, i) {
		size += sg_dma_len(sg);
		DRM_DEBUG_PRIME("importing sg table: page:%d phyaddr:0x%llx size:%d\n",
	...
	if (sgt->nents == 1)
		syna_obj->phyaddr = (dma_addr_t) sg_dma_address(syna_obj->sgt->sgl);

	if (dma_buf_begin_cpu_access(syna_obj->dma_buf, DMA_BIDIRECTIONAL) != 0) {
	...
	DRM_DEBUG_PRIME("Importing external dmabuf (%p) as gemobject\n", attach->dmabuf);
...
static int syna_gem_alloc_buf(struct drm_device *dev, struct syna_gem_object *syna_obj, bool alloc_kmap)
{
	...
	VPP_MEM_TYPE mem_type = VPP_MEM_TYPE_SYS_CUST;
#else
	VPP_MEM_TYPE mem_type = VPP_MEM_TYPE_DMA;
#endif
	if (!obj) {
		DRM_ERROR("%s %d  obj is NULL!!\n", __func__, __LINE__);
		return -EINVAL;
	...
	syna_obj->mem_type = mem_type;
	ret = VPP_MEM_AllocateMemory(dev_priv->mem_list, syna_obj->mem_type,
				&syna_obj->shm_handle, 0);
	if (ret) {
		DRM_ERROR("%s %d  gem alloc failed!\n", __func__, __LINE__);
		return -ENOMEM;
	}
	syna_obj->phyaddr = (syna_obj->mem_type == VPP_MEM_TYPE_DMA) ?
			    (phys_addr_t)syna_obj->shm_handle.p_addr : 0;
	syna_obj->kernel_vir_addr = syna_obj->shm_handle.k_addr;

	DRM_DEBUG_PRIME("Alloc memory for (%d) %lx dmabuf:%p, type:%d \n",
		        current->pid, syna_obj->phyaddr, obj->dma_buf, syna_obj->mem_type);
		...
		struct vm_area_struct *vma)
{
	...
	gem_dmabuf = syna_obj->dma_buf ? syna_obj->dma_buf :
		     (syna_obj->shm_handle.handle ? syna_obj->shm_handle.handle : NULL);

	if (gem_dmabuf) {
		ret = dma_buf_mmap(gem_dmabuf, vma, 0);
	} else {
