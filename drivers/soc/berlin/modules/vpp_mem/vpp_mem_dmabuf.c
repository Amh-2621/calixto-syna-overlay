...
static struct dma_heap *dmabuf_syscust_dma_heap;
...
static int VPP_DMABUF_InitMem_SysCust(int index)
{
        return Dmabuf_GetDmaBufByName("system_cust", &dmabuf_syscust_dma_heap);
}
...
int VPP_DMABUF_AllocMem(VPP_MEM_NODE *shm_node, VPP_MEM *shm_handle, gfp_t flags)
{
	...
	} else if (shm_node->shm_type == VPP_MEM_TYPE_SYS_CUST) {
		dmabuf_heap = dmabuf_syscust_dma_heap;
	} else {
...
int VPP_DMABUF_InitMemory(VPP_MEM_LIST *shm_list)
{
	...
	ret = VPP_DMABUF_InitMem_SysCust(0);
	if (ret != VPP_MEM_ERROR_TYPE_OK) {
		pr_err("%s: VPP_DMABUF_InitMem_SysCust FAILED ..!!!\n", __func__);
		return ret;
	}
