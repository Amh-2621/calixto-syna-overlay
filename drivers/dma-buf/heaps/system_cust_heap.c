...
struct dmabuf_page_pool *cust_pools[NUM_ORDERS];

static void *system_cust_heap_get_meta(struct dma_buf *dmabuf)
{
...
static int system_cust_heap_vmap(struct dma_buf *dmabuf, struct dma_buf_map *map)
{
	struct system_cust_heap_buffer *buffer = dmabuf->priv;
	void *vaddr;
	int ret = 0;
...
static void system_cust_heap_vunmap(struct dma_buf *dmabuf, struct dma_buf_map *map)
{
	struct system_cust_heap_buffer *buffer = dmabuf->priv;
...
static void system_cust_heap_buf_free(struct deferred_freelist_item *item,
				 enum df_reason reason)
{
	...
	buffer = container_of(item, struct system_cust_heap_buffer, deferred_free);
	...
	for_each_sgtable_sg(table, sg, i) {
		...
		if (reason == DF_UNDER_PRESSURE) {
			...
			dmabuf_page_pool_free(cust_pools[j], page);
		}
	}
	sg_free_table(table);
	kfree(buffer->meta);
...
static void system_cust_heap_free(struct dma_buf *dmabuf)
{
	struct system_cust_heap_buffer *buffer = dmabuf->priv;

	int npages = PAGE_ALIGN(buffer->len) / PAGE_SIZE;
	deferred_free(&buffer->deferred_free, system_cust_heap_buf_free, npages);
	return;
}
...
static const struct dma_buf_ops system_cust_heap_buf_ops = {
	...
	.free = system_cust_heap_free,
	.get_meta = system_cust_heap_get_meta
};
					    ...
					    unsigned int max_order)
{
	...
	for (i = 0; i < NUM_ORDERS; i++) {
		...
		if (max_order < orders[i])
			continue;
		page = dmabuf_page_pool_alloc(cust_pools[i]);
		if (!page)
			continue;
		return page;
	}
...
static int system_cust_heap_create(void)
{
	...
	for (i = 0; i < NUM_ORDERS; i++) {
		cust_pools[i] = dmabuf_page_pool_create(order_flags[i], orders[i]);
		...
		if (!cust_pools[i]) {
			...
			for (j = 0; j < i; j++)
				dmabuf_page_pool_destroy(cust_pools[j]);
			return -ENOMEM;
		}
	}
